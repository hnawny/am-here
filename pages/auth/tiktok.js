...
import { HeartHandshake, Heart, Stars, Loader2 } from 'lucide-react';
        ...
        const fetchData = async () => {
            if (code) {
                try {
                    ...
                    const response = await fetch(`/api/auth/tiktok/callback?code=${code}&state=${state}`);
      ...
      <div className="max-w-4xl w-full">
        <div className="text-center space-y-8">
          ...
          <p className="text-sm text-gray-500 mt-8">
            กำลังเชื่อมต่อกับระบบ TikTok เพื่อยืนยันตัวตนของคุณ
          </p>
        </div>
      </div>
    </div>
